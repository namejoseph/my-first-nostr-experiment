...
const getEvents = async (filter) => {
  ...
  // C( Object.keys(events).map((id) => events[id]));

    let tempVar = Object.keys(events).map((id) => events[id]);

    let newArr = [];

    for (i = 0; i < tempVar.length; i++) {
      if (tempVar[i]["kind"] == "1") {
        newArr.push(tempVar[i]["content"]);
      }
    }
  ...
  // document.getElementById("output").innerText = Object.keys(events).map((id) => JSON.stringify(events[id])).join("\n");
  // document.getElementById("output").innerText = newArr;

    let finalOutput = "";
    for (j = 0; j < newArr.length; j++) {
      finalOutput += newArr[j].replace("\n", "<br>");
      finalOutput += "<hr>";
    }
